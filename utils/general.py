...
crawl_info = {
        'max_crawl_days': 7,    #需要采集几天节目  1为当天，2为今明两天 如果频道为多天一次获取，则不受此限制
        'gen_xml_days': 10,   #需要生成节目表的天数 1当天 2明天  不能大于need_days,否则出错
        'del_days' : 14 ,     #删除多少天之前的节目
        'recrawl_days' : 1 ,  #需要重新采集几天的节目，1为只重新采集当天节目
        'retry_crawl_times' : 1 ,        #如果采集出错，重试次数 1为不重试，2为重试一次 3....重试两次
        'change_source':1, #如果获取失败是否换源
    }
dirs = {
          'share' : "%s"%root_dir,
...
def channel_ids_to_dict(channel_id):
    ...
    return channel_list
def argvs_get(argv):  #命令行运行时可使用的参数 recrawl(重新获取)  channel(单独获取某一频道节目单） dt(单独某频道时，获取日期） save_to_db单独获取时，是否需要存至数据库
...
def noepg(name,id,need_date):
    ...
    for x in range(24):
        ...
        hour_epg='''<programme start="%s" stop="%s" channel="9999"><title lang="zh">精彩节目-暂未提供节目预告信息</title><desc lang="zh"></desc></programme>'''%(starttime_str,endtime_str)
